...
class Cepstral:
  ...
  def voice_activity_detection(self, params, base_filename):
    #########################
    ## Initialisation part ##
    #########################
    ...
    if useMod4Hz:
      ...
      X = numpy.ndarray(shape=(normalized_energy.shape[0]+normalized_Mod4Hz.shape[0],len(normalized_energy)), dtype=numpy.float64)
    ...
    out_params = numpy.ndarray(shape=((label == 1).sum(),len(features_mask)), dtype=numpy.float64)
    i=0
    cur_i=0
    ...
    for row in params:
      if label[i]==1:
        for k in range(len(features_mask)):
          out_params[cur_i,k] = row[features_mask[k]]
        cur_i = cur_i + 1
      i = i+1
    print i
    ...
    if self.m_config.withVADFiltering:
      ...
      if self.m_config.useExistingVAD:
        ...
        if os.path.exists(vad_file):
          filtered_features = self.use_existing_vad(cepstral_features, vad_file)
        else:
          print "Warning: couldn't retreive the existing vad file " + vad_file + ", will try to do the VAD instead"
          filtered_features = self.voice_activity_detection(cepstral_features, base_filename)
