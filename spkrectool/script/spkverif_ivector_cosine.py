...
class ToolChainExecutorZT (ToolChainExecutor.ToolChainExecutor):
  ...
  def execute_tool_chain(self):
    ...
    if not self.m_args.skip_score_computation:
      self.m_tool_chain.compute_scores(self.m_tool, not self.m_args.no_zt_norm, dir_type=cur_type, 
                                       groups = self.m_args.groups, preload_probes = self.m_args.preload_probes, 
                                       force = self.m_args.force, scoring_type='cosine')
