...
class ToolChainExecutorZT (ToolChainExecutor.ToolChainExecutor):
  ...
  def execute_tool_chain(self):
    ...
    if not self.m_args.skip_lda_train_projector and hasattr(self.m_tool, 'lda_train_projector'):
      self.m_tool_chain.lda_train_projector(self.m_tool, dir_type='projected_ivector', force = self.m_args.force)
    ...
    if not self.m_args.skip_lda_projection and hasattr(self.m_tool, 'lda_project_ivector'):
      self.m_tool_chain.lda_project_ivector(self.m_tool, dir_type='projected_ivector', force = self.m_args.force)  
    ...
    if not self.m_args.skip_whitening_enroler_training and hasattr(self.m_tool, 'train_whitening_enroler'):
      self.m_tool_chain.train_whitening_enroler(self.m_tool, dir_type='wccn_projected_ivector', force = self.m_args.force)
    
    # whitening i-vectors
    if not self.m_args.skip_whitening_ivector and hasattr(self.m_tool, 'whitening_ivector'):
      self.m_tool_chain.whitening_ivector(self.m_tool, dir_type='wccn_projected_ivector', force = self.m_args.force)

          
    # train plda enroler
    print "Train plda enroler"
    if not self.m_args.skip_plda_enroler_training and hasattr(self.m_tool, 'train_plda_enroler'):
      self.m_tool_chain.train_plda_enroler(self.m_tool, dir_type='whitened_ivector', force = self.m_args.force)
    
    # PLDA enrollment of the models
    print "Enrol models with PLDA"
    if not self.m_args.skip_model_enrolment:
      self.m_tool_chain.enrol_models(self.m_tool, self.m_feature_extractor, not self.m_args.no_zt_norm, dir_type='whitened_ivector', groups = self.m_args.groups, force = self.m_args.force)
    
      
    # score computation
    if not self.m_args.skip_score_computation:
      self.m_tool_chain.compute_scores(self.m_tool, not self.m_args.no_zt_norm, dir_type='whitened_ivector', groups = self.m_args.groups, preload_probes = self.m_args.preload_probes, force = self.m_args.force)
  ...
  def add_jobs_to_grid(self, external_dependencies):
    ...
    # train PLDA
    if not self.m_args.skip_plda_enroler_training and hasattr(self.m_tool, 'train_plda_enroler'):
      job_ids['plda_enrolment_training'] = self.submit_grid_job(
              '--train-plda-enroler', 
              name = "plda-e-training",
              dependencies = deps, 
              **self.m_grid_config.training_queue)
      deps.append(job_ids['plda_enrolment_training'])
