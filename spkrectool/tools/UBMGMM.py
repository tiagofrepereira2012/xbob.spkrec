...
class UBMGMMTool:
  ...
  def train_projector(self, train_files, projector_file):
    ...
    #train_features = [bob.io.load(str(train_files[k])) for k in range(len(train_files))]
    train_features = []
    for k in range(len(train_files)):
      x = bob.io.load(str(train_files[k]))
      #print x.shape, x.shape[0], len(x.shape)
  ...
  def _project_using_array(self, array):
    print " .... Projecting %d feature vectors" % array.shape[0]
  ...
  def project_ubm(self, feature_array):
  ...
  def score(self, model, probe):
    ...
    return self.m_scoring_function([model], self.m_ubm, [probe], list(), True)
