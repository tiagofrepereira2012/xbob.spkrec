...
class FileSelectorZT:
  ...
  def zprobe_objects_for_model(self, model_id, group):
    """Returns the probe objects used to compute the Z-Norm"""
    #return self.m_db.zobjects(directory=self.__probe_dir__(use_projected_ubm_dir, use_projected_isv_dir), extension=self.m_config.default_extension, protocol=self.m_config.protocol, groups=group, model_ids=(model_id,))
