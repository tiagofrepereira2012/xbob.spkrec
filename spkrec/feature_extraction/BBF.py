...
class BBF:
  ...
  def __call__(self, input_file, vad_file=None):
    ...
    spectro = bob.ap.Spectrogram(rate_wavsample[0], wl, ws, nf, f_min, f_max, pre)
    spectro.dct_norm = self.m_config.dct_norm
    spectro.mel_scale = self.m_config.mel_scale
    spectro.with_energy = self.m_config.withEnergy
    spectro.with_delta = self.m_config.withDelta
    spectro.with_delta_delta = self.m_config.withDeltaDelta
       
    spectral_features = spectro(rate_wavsample[1] )
 
    # Voice activity detection
    if vad_file is None:
      labels=label = numpy.array(numpy.ones(spectral_features.shape[0]), dtype=numpy.int16)
    else:
